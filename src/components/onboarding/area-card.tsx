...
export function AreaCard({ area, parentName, isSelected, onClick }: AreaCardProps) {
  ...
  return (
    ...
    >
      {isSelected && (
        <div className="absolute right-3 top-3 flex h-6 w-6 items-center justify-center rounded-full bg-[#005EB8]">
          <Check className="h-4 w-4 text-white" />
        </div>
      )}
      <div className="flex items-center gap-2 pr-8">
        <span className={cn('font-semibold', isSelected ? 'text-[#005EB8]' : 'text-gray-900')}>
          {displayName}
        </span>
