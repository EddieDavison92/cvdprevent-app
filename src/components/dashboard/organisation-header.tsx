  ...
  return (
    <div className="flex min-w-0 items-start gap-3">
      ...
      <div className="min-w-0 flex-1">
        <h1 className="text-pretty text-xl font-bold leading-tight text-nhs-dark-blue sm:text-2xl">{displayName}</h1>
        <p className="text-sm text-gray-500">
          {isEngland ? 'National overview' : SYSTEM_LEVEL_NAMES[organisation.SystemLevelID]}
        </p>
      </div>
      <AreaChangeDialog compact className="ml-auto shrink-0 sm:hidden" />
    </div>
  );
}
